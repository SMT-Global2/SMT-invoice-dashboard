...
>(
  (
    ...
    const isHoverOpenRef = React.useRef(false)
    const hoverTimeoutRef = React.useRef<ReturnType<typeof setTimeout>>(undefined)

    const handleMouseEnter = React.useCallback(() => {
      if (hoverTimeoutRef.current) clearTimeout(hoverTimeoutRef.current)
      if (!open && collapsible === "icon" && !isMobile) {
        isHoverOpenRef.current = true
        setOpen(true)
      }
    }, [open, collapsible, isMobile, setOpen])

    const handleMouseLeave = React.useCallback(() => {
      if (isHoverOpenRef.current) {
    ...
    if (collapsible === "none") {
      return (
        <div
          className={cn(
            "flex h-full w-[--sidebar-width] flex-col bg-sidebar text-sidebar-foreground overflow-y-auto h-screen",
            className
          )}
          ref={ref}
          {...props}
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm whitespace-nowrap outline-none ring-sidebar-ring transition-[width,height,padding] duration-300 ease-in-out hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-medium data-[active=true]:text-sidebar-accent-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:!size-8 group-data-[collapsible=icon]:!p-2 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
  {
    variants: {
      variant: {
